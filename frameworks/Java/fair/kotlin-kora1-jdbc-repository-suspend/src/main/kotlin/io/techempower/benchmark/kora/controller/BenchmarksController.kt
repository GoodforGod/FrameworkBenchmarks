...
class BenchmarksController(private val repository: WorldRepository) {
    ...
    suspend fun db(): World = repository.findById(QueryUtils.randomWorld())

    @Json
    @HttpRoute(method = HttpMethod.GET, path = "/queries")
    suspend fun queries(@Query("queries") queries: String?): List<World> {
    ...
    suspend fun updates(@Query("queries") queries: String?): List<World> {
        val count = QueryUtils.parseCount(queries)
        val worlds = ArrayList<World>(count)
        repeat(count) {
