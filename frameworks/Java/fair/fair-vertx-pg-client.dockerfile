...
RUN gradle java-vertx-pg-client:distTar --no-daemon
...
ENV POSTGRES_PIPELINING_LIMIT="1"

COPY --from=builder /home/gradle/src/java-vertx-pg-client/build/distributions/application.tar /app/application.tar
