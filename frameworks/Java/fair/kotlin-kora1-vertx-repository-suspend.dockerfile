...
RUN gradle kotlin-kora1-vertx-repository-suspend:distTar --no-daemon

FROM eclipse-temurin:25-jre-jammy AS runner

WORKDIR /app

ENV DEFAULT_JVM_OPTS="-XX:+UseParallelGC -XX:+UseNUMA -Djava.net.preferIPv4Stack=true -Djdk.trackAllThreads=false -XX:AutoBoxCacheMax=11000 -XX:InitialCodeCacheSize=256m -XX:ReservedCodeCacheSize=256m -XX:MaxInlineLevel=20 -XX:-StackTraceInThrowable -XX:+UseCompressedClassPointers"
...
COPY --from=builder /home/gradle/src/kotlin-kora1-vertx-repository-suspend/build/distributions/application.tar /app/application.tar
